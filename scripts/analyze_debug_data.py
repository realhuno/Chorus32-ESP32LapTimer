...
def plot_data(data: List[int], threshold, grace_samples, lowpass: lowpass_filter):
	...
	ax[0].plot(t, lower_data, t, upper_data)
	ax[0].plot(t, lower_data_filtered, t, upper_data_filtered)
	ax[0].axhline(y=threshold, color='r')
	ax[0].axvline(x=raw_max[1], color='r', label="raw max: difference {0:.2f}ms".format(abs(raw_max[1] - filtered_max[1]) * (1/args.sampling) * 1000))
	ax[0].axvline(x=filtered_max[1], label="filtered max: difference {} samples".format(abs(raw_max[1] - filtered_max[1])))
	ax[0].set_xlabel('Sample')
	ax[0].set_ylabel('Amplitude')
	ax[0].legend()

	ax[1].magnitude_spectrum(normalized_data, Fs=args.sampling, scale=args.scale)
...
if __name__ == "__main__":
	...
	for data in data_list:
		plot_data(data, args.threshold * 12, 400, lowpass_filter(args.cutoff, 1/args.sampling))
	plt.show()
